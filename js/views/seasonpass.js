import { getSeasonPassLevels, currentSeasonPassLevel, listCosmetics, listBanners } from "../data.js";
...
export async function render(container, ctx) {
  ...
  const cosmeticsByLevel = new Map(cosmetics.map(c => [c.level, c]));
  const bannersByLevel = new Map(banners.map(b => [b.level, b]));
  ...
  const hasEquippable = cosmetics.length > 0 || banners.length > 0;

  function rowHTML(l, isUnlocked) {
    const cosmetic = cosmeticsByLevel.get(l.level);
    const banner = bannersByLevel.get(l.level);
    const rewardDisplay = cosmetic
      ? `<span style="font-size:1.2rem;margin-right:6px">${cosmetic.icon}</span>${cosmetic.name}`
      : banner
        ? `<span style="display:inline-block;width:36px;height:14px;border-radius:3px;vertical-align:middle;margin-right:6px;background:${banner.color}"></span>${banner.name}`
        : l.reward;
    return `
  ...
  container.innerHTML = `
    <h2>Season Pass</h2>
    <p class="muted">Free for everyone, ${levels.length} levels this season. Earn XP to unlock cosmetics — nothing here affects your Ranked score, and everything you unlock is yours permanently.
      ${hasEquippable ? ` Head to your <a href="#/profile" style="color:var(--red-bright)">Profile</a> to equip anything you've unlocked.` : ""}</p>
