...
export async function createTournament(data, actorUid) {
  const ref = await addDoc(collection(db, "tournaments"), { ...data, status: "active", createdAt: serverTimestamp() });
...
// Ends a tournament: no more attempts can be submitted, but existing
// attempts and the leaderboard remain visible/gradeable.
export async function endTournament(tournamentId, actorUid) {
  await updateDoc(doc(db, "tournaments", tournamentId), { status: "ended", endedAt: serverTimestamp() });
  await logAudit(actorUid, "tournament_ended", tournamentId, "active", "ended");
}
...
export async function updateOwnProfile(uid, fields) {
  ...
  const allowed = ["displayName", "block", "instruments", "profilePicUrl"];
  const clean = {};
...
// exactly which level a student has reached.
